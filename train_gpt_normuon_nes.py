        ...
        for group, info in zip(self.param_groups, group_infos):
            ...
            for i in range(chunk_size):
                ...
                if grad.size(-2) <= grad.size(-1):
                    norm1 = update_grad.norm(dim=-1, keepdim=True)  # shape (N, 1)
                    norm = grad.norm(dim=-1, keepdim=True)
                    grad.mul_(0.85 * norm1 / (1e-8 + norm))
                    update_grad = torch.cat([update_grad, grad], dim=-2)
                    cat_dim = -2
                else:
                    norm1 = update_grad.norm(dim=-2, keepdim=True)  # shape (N, 1)
                    norm = grad.norm(dim=-2, keepdim=True)
                    grad.mul_(0.85 * norm1 / (1e-8 + norm)) 
                    update_grad = torch.cat([update_grad, grad], dim=-1)
                    cat_dim = -1
