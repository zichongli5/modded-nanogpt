        ...
        for group in self.param_groups:
            ...
            for base_i in range(0, len(params), world_size):
                ...
                if base_i + rank < len(params):
                    ...
                    grad1 = grad.lerp(momentum_buffer, momentum)
                    # grad = grad.lerp_(momentum_buffer, momentum)
                    if  p.size(-2) <= p.size(-1):

                        dim = grad1.size(-2)
                        norm1 = grad1.norm(dim=-1, keepdim=True)  # shape (N, 1)
                        norm = grad.norm(dim=-1, keepdim=True)
                        grad.mul_(0.85 * norm1 / (1e-8 + norm)) 

                        grad = torch.cat([grad1, grad], dim=-2)
                        v = zeropower_via_newtonschulz5(grad.bfloat16(), 5).float()[..., :dim, :]
                    else:
                        dim = grad1.size(-1)
                        norm1 = grad1.norm(dim=-2, keepdim=True)  # shape (N, 1)
                        norm = grad.norm(dim=-2, keepdim=True)
                        grad.mul_(0.85 * norm1 / (1e-8 + norm)) 

                        grad = torch.cat([grad1, grad], dim=-1)
                        v = zeropower_via_newtonschulz5(grad.bfloat16(), 5).float()[..., :dim]
...
class Hyperparameters:
    # data
    train_files = '/home/workspace/dataset/fineweb10B/fineweb_train_*.bin' # input .bin to train on
    val_files = '/home/workspace/dataset/fineweb10B/fineweb_val_*.bin' # input .bin to eval validation loss on
    val_tokens = 10485760 # how many tokens of validation data? it's important to keep this fixed for consistent comparisons
    train_seq_len = 48*1024 # FlexAttention sequence length
    val_seq_len = 4*64*1024 # FlexAttention sequence length for validation
    # optimization
    num_iterations = 1680 # number of iterations to run
...
if master_process:
    ...
    run = wandb.init(project="MGPT", config=vars(args), name="124M_Muon_latest_1685")
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        if master_process:
            wandb.log({"step": step, "val_loss": val_loss.item(), "train_time_ms": training_time_ms, "val_step_avg_ms": training_time_ms/max(step, 1)})
        model.train()
